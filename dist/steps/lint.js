...
import { GEMINI_API_KEY } from "../misc/key.js";
...
export async function lint(files, options) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        const lintRes = await lintFile(file, lintSpinner, {
            current: i + 1,
            total: files.length,
        }, options);
        if (lintRes) {
    ...
    for (const change of changes) {
        ...
        for (let i = 0; i < og_by_line.length && i < new_by_line.length; i++) {
            ...
            if (ogLine !== newLine) {
                issueCount++;
                diffString += `${chalk.bgRed.white("- " + ogLine)}\n${chalk.bgGreen.white("+ " + newLine)}\n`;
            }
        }
        if (diffString !== "") {
            ...
            console.log(chalk.bgBlack.white(` No changes found in ${change.relative_path} `));
        }
    }
...
async function lintFile(file, spinner, progress, options) {
    spinner.update({
            ...
            ? [
                {
                    ...
                    parts: [
                        {
                            text: "Before you start processing files, here are some notes and additional instructions.\n" +
                                options.llmnote,
                        },
                    ],
                },
    ...
    try {
        ...
        for (const correction of parsed) {
            if (fileContents.includes(correction.original)) {
                ...
                newFileContents = newFileContents.replace(correction.original, correction.corrected);
            }
            else {
