...
async function selectDirectory(workingDirectory) {
    const directories = fs
        .readdirSync(workingDirectory)
        .filter((item) => fs.lstatSync(`${workingDirectory}/${item}`).isDirectory());
    const answer = await select({
        message: `${chalk.magentaBright("Which directory do you want to lint? (select root for a sub-directory)")}`,
        choices: [
    ...
    for (const file of files) {
        if (!file.isDirectory()) {
            foundFiles.push({
                ...
                relative_path: `${dir}/${file.name}`.replace(`${directoryToCheck}/`, ""),
            });
        }
        else {
...
export async function stepMain(options) {
    ...
    if (!directoryToCheck) {
        ...
        console.log(`${chalk.magentaBright(`Selected directory: ${directoryToCheck}`)}`);
    }
    ...
    console.log(`${chalk.magentaBright(`Selected filetype: ${fileTypeToCheck}`)}`);
    ...
    checkFilesSpinner.success({
        text: `${chalk.magentaBright(`Found ${foundFiles.length} ${foundFiles.length == 1 ? "file" : "files"} to check.`)}`,
    });
    console.log(chalk.magenta(` Directory: ${directoryToCheck}
